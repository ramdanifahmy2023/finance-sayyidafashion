  ...
  const setQuickFilter = (filterType: string) => {
    ...
    switch (filterType) {
      ...
      case 'last7days':
        ...
        break;
      case 'thisMonth':
        setDateRange({ from: thisMonthStart, to: today });
        break;
      case 'lastMonth':
  ...
  const filteredSales = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter(sale => 
        ...
        formatProductType(sale.product_type).toLowerCase().includes(searchLower) ||
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder="Cari berdasarkan pelanggan, produk, atau metode pembayaran..."
                  value={searchTerm}
                ...
                {(dateRange.from || dateRange.to) && (
                  <Badge variant="secondary" className="flex items-center gap-1">
                    ...
                    {dateRange.from && dateRange.to && dateRange.from.toDateString() === dateRange.to.toDateString()
                      ? `Tanggal: ${dateRange.from.toLocaleDateString('id-ID')}`
            ...
            <div className="flex justify-between items-center text-sm text-muted-foreground">
              ...
              {filteredSales.length === 0 && sales.length > 0 && (
                <span className="text-orange-600">Tidak ada data yang sesuai dengan filter</span>
              )}
            </div>
          </div>
        </CardContent>
