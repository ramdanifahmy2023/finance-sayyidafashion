  ...
  const loadExpenses = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('expenses').select('*').order('transaction_date', {
        ascending: false
      });
          ...
          <CardContent>
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <IDRInput label="Jumlah (IDR)" value={formData.amount} onChange={value => setFormData(prev => ({
              ...prev,
              amount: value
              ...
              }))} placeholder="Additional notes about this expense..." rows={3} />
                </div>
              </div>
                  ...
                  {expenses.map(expense => <TableRow key={expense.id}>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          ...
                          <Button size="sm" variant="ghost" onClick={() => handleDelete(expense.id)} className="text-rose-600">
